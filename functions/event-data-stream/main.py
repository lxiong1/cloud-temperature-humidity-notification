...
def send_data_to_firestore(event, context):
    ...
    climate_document = {
        ...
        else "relativeHumidity": event_data,
    ...
    if threshold_reached is True:
        publish_threshold_event(craft_topic_message(event_name, event_data), event_name)
...
def publish_threshold_event(topic_message, event_name):
    ...
    publisher_client.publish(
        publisher_client.topic_path(PROJECT_ID, topic_name), topic_message
    )

    print(
        f'{event_name.capitalize()} threshold has been reached, publishing message to topic "{topic_name}"'
    )
